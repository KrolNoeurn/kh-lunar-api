...
export const integerToWord = (num) => {
    let word = '';
    let remainder = 0;
...
export const integerToCurrencyWord = (num) => {
  let word = '';
  let remainder = 0;
  if (num < TEN) {  //9
      word = SINGLE_DIGIT[num];

  } else if (num < ONE_HUNDRED) { //99
      remainder = num % TEN;
      word = DOUBLE_DIGIT[Math.floor(num / TEN)];  

  } else if (num < ONE_THOUSAND) { //999
      remainder = num % ONE_HUNDRED;
      word = SINGLE_DIGIT[Math.floor(num / ONE_HUNDRED)] + CurrencyWords.HUNDRED;
  } else if (num < ONE_MILLION) {

      remainder = num % ONE_THOUSAND;
      word = integerToCurrencyWord(Math.floor(num / ONE_THOUSAND)) + CurrencyWords.THOUSAND;
  } else if (num < ONE_BILLION) {
      remainder = num % ONE_MILLION;
      word = integerToCurrencyWord(Math.floor(num / ONE_MILLION)) + CurrencyWords.MILLION;
  } else {
      throw new Error('Amount too large to convert to words');
  }   
  if (remainder !== 0) {
      return `${word}${Separator}${integerToCurrencyWord(remainder)}`;
  }
  return word;
} 